...
ModuleEnrichment <- function(module_list, human_or_mouse){
        ...
        return(list('GO' = modules.go, 'Reactome' = modules.reactome))
}
...
CountCellBarPlot <- function(srt_obj, group.var = NULL, stack.var = NULL,
                             stack.color = mycol_14, percentage = F, width = 0.7, ...){
...
MarkerVolcano <- function(mk.df, label = T, min.log2FC = 0.25, label_genes = NULL, max.overlaps = 10, ...){
        df <- mk.df
        df$gene <- rownames(df)
        ...
        df$Significance[df$log_p >= 2 & abs(df$avg_log2FC) >= min.log2FC] <- 'Adjusted p < 0.01'
        df$Significance <- factor(df$Significance, levels = c('Adjusted p < 0.01', 'Not sig.'))
        ...
        if(label){
                p <- p +
                        ggrepel::geom_text_repel(data = df_label, aes(label = gene, x = avg_log2FC, y = log_p),
                                                 color = 'black', size = 3,
                                                 max.overlaps = max.overlaps, min.segment.length = 0.3, ...)
        }
        return(p)
}
...
DownsampleByMeta <- function(srt, meta_var, n, down_to_min_group = F, random = F){
        cells <- split(Cells(srt), srt[[meta_var]])
        if(down_to_min_group){n = min(lengths(cells))}
        for(i in 1:L(cells)){
                if(L(cells[[i]]) > n){
                        if(random){
                                cells[[i]] <- sample(cells[[i]], size = n, replace = F)
                        } else {
...
AddModuleScore2 <- function(srt, features, assay = NULL, names, return_z = F, ...){
...
EmbeddingMappingCorr <- function(ref_seurat, ref_assay = NULL, que_seurat, que_assay = NULL, features){
        ...
        for(i in 1:ncol(ref_seurat)){
                ...
                for(j in 1:ncol(que_seurat)){
                        pcc_mat[i, j] <- cor(ref_exp[, i], que_exp[, j])
                }
        }
...
EmbeddingMappingPlot <- function(ref_seurat, ref_cols = mycol_20, ref_group_by = 'ident', ref_pt_size = 0.5,
                                 que_seurat, que_cols = 'black', que_group_by = 'Query', que_pt_size = 1,
                                 ref_reduction = 'umap', k = 20, pcc_mat) {
        if(nrow(pcc_mat) != ncol(ref_seurat) | ncol(pcc_mat) != ncol(que_seurat)){
                stop('pccc matrix does not match seurat')}
        ...
        p <- ggplot(data = combine_emb) +
                geom_point(aes(x = X, y = Y, color = Group, size = Size)) +
