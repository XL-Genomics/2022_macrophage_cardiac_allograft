...
##  Plot NK cells
colors <- mycol_10[1:6]
lym.srt <- srt[, srt$Cell_type == 'Lym' & srt$Cell_type_non_ambig]
lym.srt <- lym.srt[, !is.na(lym.srt@reductions$sub_umap@cell.embeddings[,1])]
DimPlot2(lym.srt, group.by = 'Cell_state', cols = colors, reduction = 'sub_umap')

srtLoom_lym <- srtLoom[, Cells(lym.srt)]
srtLoom_lym$Cell_state <- droplevels(srtLoom_lym$Cell_state)

names(colors) <- levels(srtLoom_lym$Cell_state)
cell.colors <- colors[srtLoom_lym$Cell_state[colnames(srtLoom_lym)]]
names(cell.colors) <- colnames(srtLoom_lym)

lym.vlc <- show.velocity.on.embedding.cor(emb = Embeddings(lym.srt, reduction = "sub_umap"),
                                          vel = Tool(srtLoom_lym, slot = "RunVelocity"),
                                          scale = 'sqrt',
                                          n.cores = 16)

PlotPDF('02.velocity.p136_lymphoid_sub_umap_projection', 8, 8)
show.velocity.on.embedding.cor(emb = Embeddings(lym.srt, reduction = "sub_umap"),
                               vel = Tool(srtLoom_lym, slot = "RunVelocity"),
                               cc = lym.vlc$cc,
                               cell.colors = ac(cell.colors, alpha = 0.5),
                               cell.border.alpha = 0.04,
...
saveRDS(lym.vlc, 'analysis/STEP21.p136_lymphoid_velocity_embedding.vlc.rds')
saveRDS(srtLoom_imm, 'analysis/STEP21.p136_myeloid_velocity.srt.rds')
saveRDS(srtLoom_lym, 'analysis/STEP21.p136_lymphoid_velocity.srt.rds')
...
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####  Plot P136 myeloid velocity on diffusion map  ####
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
srt <- readRDS('integrated/STEP18.annotated.srt.rds')
...
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####  Plot P136 lymphoid velocity on diffusion map  ####
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
srt <- readRDS('integrated/STEP18.annotated.srt.rds')
nk.vlc <- readRDS('analysis/STEP21.p136_myeloid_velocity_embedding.vlc.rds')
srtLoom_imm <- readRDS('analysis/STEP21.p136_myeloid_velocity.srt.rds')
mye.dif <- readRDS('analysis/STEP20.myeloid_diffusion_result.rds')
